...
import com.example.sensorlogger.model.GnssRawSnapshot
import com.example.sensorlogger.model.GnssRawMeasurement
...
class GnssManager(private val context: Context) {
    ...
    private var lastRawSnapshot: GnssRawSnapshot? = null
    private var firstFixTimeMillis: Long? = null
    ...
    private val locationCallback = object : LocationCallback() {
        override fun onLocationResult(result: LocationResult) {
            ...
            synchronized(lock) {
                ...
                if (firstFixTimeMillis == null && location.hasAccuracy()) {
                    firstFixTimeMillis = SystemClock.elapsedRealtime()
                }
            }
        }
    }
    ...
    private val measurementsCallback = object : GnssMeasurementsEvent.Callback() {
        override fun onGnssMeasurementsReceived(eventArgs: GnssMeasurementsEvent) {
            synchronized(lock) {
                ...
                val measurements = mutableListOf<GnssRawMeasurement>()
                val cn0Values = mutableListOf<Double>()
                val agcValues = mutableListOf<Double>()
                val dopplerSpeeds = mutableListOf<Double>()
                
                val clock = eventArgs.clock
                val clockBiasNanos = if (clock.hasBiasNanos()) clock.biasNanos.toLong() else null
                val clockDriftNanosPerSecond = if (clock.hasDriftNanosPerSecond()) 
                    clock.driftNanosPerSecond else null
                ...
                eventArgs.measurements.forEach { measurement ->
                    ...
                    val carrierFreq = if (measurement.hasCarrierFrequencyHz()) 
                        measurement.carrierFrequencyHz.toDouble() else null
                    
                    // Extract CN0
                    val cn0 = measurement.cn0DbHz.let { 
                        if (it > 0.0) it else null 
                    }
                    cn0?.let { cn0Values.add(it) }
                    
                    // Extract pseudorange rate
                    val pseudorangeRate = measurement.pseudorangeRateMetersPerSecond.let {
                        if (it.isFinite()) it else null
                    }
                    
                    // Calculate Doppler-based speed
                    if (pseudorangeRate != null && carrierFreq != null && carrierFreq > 0) {
                        val speedMps = abs(pseudorangeRate)
                        dopplerSpeeds.add(speedMps)
                    }
                    
                    // Extract accumulated delta range
                    val adr = measurement.accumulatedDeltaRangeMeters.let {
                        if (it.isFinite()) it else null
                    }
                    val adrState = measurement.accumulatedDeltaRangeState.let {
                        if (it != 0) it else null
                    }
                    
                    // Extract multipath indicator
                    val multipath = measurement.multipathIndicator.let {
                        if (it != 0) it else null
                    }
                    ...
                    val agc = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && 
                        ...
                        measurement.automaticGainControlLevelDb.toDouble().also { 
                            agcValues.add(it) 
                        }
                    } else null
                    
                    // Try to extract SNR via reflection (some devices expose it)
                    val snr = try {
                        val field = measurement.javaClass.getDeclaredField("mSnrInDb")
                        field.isAccessible = true
                        val value = field.getDouble(measurement)
                        if (value != 0.0) value else null
                    } catch (_: Throwable) {
                        null
                    }
                    
                    measurements.add(GnssRawMeasurement(
                        svid = measurement.svid,
                        constellationType = measurement.constellationType,
                        carrierFrequencyHz = carrierFreq,
                        cn0DbHz = cn0,
                        pseudorangeRateMetersPerSecond = pseudorangeRate,
                ...
                val cn0Avg = cn0Values.takeIf { it.isNotEmpty() }?.average()
                val cn0Min = cn0Values.minOrNull()
                val cn0Max = cn0Values.maxOrNull()
                val cn0Sigma = if (cn0Values.size > 1) {
                    val mean = cn0Avg ?: 0.0
                    sqrt(cn0Values.map { (it - mean).pow(2) }.average())
                } else null
                
                val agcAvg = agcValues.takeIf { it.isNotEmpty() }?.average()
                val agcMin = agcValues.minOrNull()
                val agcMax = agcValues.maxOrNull()
                
                val dopplerSpeedMps = dopplerSpeeds.takeIf { it.isNotEmpty() }?.average()
                val dopplerSpeedSigma = if (dopplerSpeeds.size > 1) {
                    val mean = dopplerSpeedMps ?: 0.0
                    sqrt(dopplerSpeeds.map { (it - mean).pow(2) }.average())
                } else null
                
                val timeToFirstFixMs = firstFixTimeMillis?.let { 
                    (SystemClock.elapsedRealtime() - startTimeMillis).toDouble()
                }
                
                val satUpdateAgeMs = SystemClock.elapsedRealtime() - 
                    (lastElapsedRealtimeNanos / 1_000_000L)
                
                lastRawSnapshot = GnssRawSnapshot(
                    timestampNanos = clock.timeNanos,
                    measurements = measurements,
                    dopplerSpeedMps = dopplerSpeedMps,
                    dopplerSpeedSigma = dopplerSpeedSigma,
                    agcDbAvg = agcAvg,
                    agcDbMin = agcMin,
                    agcDbMax = agcMax,
                    cn0Avg = cn0Avg,
                    ...
                    cn0Max = cn0Max,
                    cn0Sigma = cn0Sigma,
                    dopplerSatCount = dopplerSpeeds.size.takeIf { it > 0 },
                    clockBiasNanos = clockBiasNanos,
                    clockDriftNanosPerSecond = clockDriftNanosPerSecond,
                    timeToFirstFixMs = timeToFirstFixMs,
                    satUpdateAgeMs = satUpdateAgeMs
                )
            }
        }
