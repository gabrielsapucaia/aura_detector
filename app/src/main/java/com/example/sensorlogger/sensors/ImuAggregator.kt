...
class ImuAggregator(context: Context) : SensorEventListener {
    ...
    fun snapshot(nowMillis: Long): ImuSnapshot = synchronized(lock) {
        ...
        // Calcular acelerações compensadas pela orientação (earth frame)
        val linearAccStats = bucket.buildLinearAccelerationStats()
        val earthFrameAcc = if (latestQuaternion != Quaternion.IDENTITY && 
                                linearAccStats.x.mean != null &&
                                linearAccStats.y.mean != null &&
                                linearAccStats.z.mean != null) {
            com.example.sensorlogger.util.ImuMath.computeEarthFrameAcceleration(
                quaternion = latestQuaternion,
                linAx = linearAccStats.x.mean!!,
                linAy = linearAccStats.y.mean!!,
                linAz = linearAccStats.z.mean!!
            )
        } else {
            com.example.sensorlogger.util.EarthFrameAcceleration(null, null, null)
        }
        ...
        val snapshot = ImuSnapshot(
            ...
            linearAccelerationStats = linearAccStats,
            ...
            accLongitudinalMps2 = earthFrameAcc.accLongitudinal,
            accLateralMps2 = earthFrameAcc.accLateral,
            accVerticalMps2 = earthFrameAcc.accVertical,
            vehicleTiltPitchDeg = pitch.takeIf { it.isFinite() },
            vehicleTiltRollDeg = roll.takeIf { it.isFinite() }
        )
        bucket = Bucket()
...
internal class StatsAccumulator {
    ...
    fun sigma(): Float? {
        ...
        val sumSquaredDiff = sumSq - meanVal * meanVal * count
        if (sumSquaredDiff <= 0.0) {
            return 0f
        }
        val variance = sumSquaredDiff / (count - 1)
